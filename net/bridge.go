...
func EnforceAddrAssignType(root, bridgeName string) error {
	sysctlFilename := filepath.Join(root, "/sys/class/net/", bridgeName, "/addr_assign_type")
...
func CreateBridge(root string, config *BridgeConfig) (BridgeType, error) {
	...
	if config.IsAWSVPC {
		...
		if err := sysctl(root, "net/ipv4/conf/"+config.WeaveBridgeName+"/proxy_arp", "1"); err != nil {
		...
		if err := sysctl(root, "net/ipv4/neigh/"+config.WeaveBridgeName+"/proxy_delay", "0"); err != nil {
			return bridgeType, errors.Wrap(err, "setting proxy_arp")
		}
	}
	...
	if err := ConfigureARPCache(root, config.WeaveBridgeName); err != nil {
...
func initFastdp(config *BridgeConfig) error {
	...
	if err := netlink.LinkSetMTU(datapath, config.MTU); err != nil {
		return errors.Wrapf(err, "setting datapath %q mtu %d", config.WeaveBridgeName, config.MTU)
	}
	return nil
}
